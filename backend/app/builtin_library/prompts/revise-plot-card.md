...
{# Show what the card holds today, read straight from the fields registered
   above so this can't drift from what the commit writes. Body (the synopsis) and
   title are shown above, so skip them here. A long-form field gets its own block
   so its text isn't crammed onto a bullet line; everything else is a bullet. #}
