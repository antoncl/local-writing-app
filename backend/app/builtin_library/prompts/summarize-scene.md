...
{% role "system" %}
{# Register the field this prompt writes. The commit reads this same set back
   and writes exactly it — nothing else — so register every field you want the
   author to be able to commit, and only those. Here that is just `summary`.
   The loop emits no text. #}
{% for f in fields(e) if f.id == "summary" %}{% do field_contract.store(f) %}{% endfor %}
