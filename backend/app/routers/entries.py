...
def list_views(project: CurrentProject) -> ViewNodeList:
    """Saved views (0.5.0, #35/#78) — frontmatter-only `view` nodes."""
    with translate_errors():
        return project.list_views()


@router.post("/api/views", response_model=ViewNode)
def create_view(project: CurrentProject, request: CreateViewRequest) -> ViewNode:
    with translate_errors():
        return project.create_view(request)


@router.get("/api/views/{view_id}", response_model=ViewNode)
def get_view(project: CurrentProject, view_id: str) -> ViewNode:
    with translate_errors():
        return project.read_view(view_id)


@router.put("/api/views/{view_id}", response_model=ViewNode)
def save_view(project: CurrentProject, view_id: str, request: SaveViewRequest) -> ViewNode:
    with translate_errors():
        return project.save_view(view_id, request)


@router.put("/api/views/{view_id}/ui", response_model=ViewNode)
def update_view_ui(project: CurrentProject, view_id: str, request: UpdateViewUiRequest) -> ViewNode:
    ...
    save."""
    with translate_errors():
        return project.update_view_ui(view_id, request)
...
def list_assistant_entries(project: CurrentProject) -> AssistantEntryList:
    with translate_errors():
        return project.list_assistant_entries()


@router.post("/api/assistants", response_model=AssistantEntry)
def create_assistant_entry(project: CurrentProject, request: CreateAssistantEntryRequest) -> AssistantEntry:
    with translate_errors():
        return project.create_assistant_entry(request)


@router.get("/api/assistants/{entry_id}", response_model=AssistantEntry)
def get_assistant_entry(project: CurrentProject, entry_id: str) -> AssistantEntry:
    with translate_errors():
        return project.read_assistant_entry(entry_id)
...
def save_assistant_entry(project: CurrentProject, entry_id: str, request: SaveAssistantEntryRequest) -> AssistantEntry:
    ...
    with translate_errors():
        return project.save_assistant_entry(entry_id, request)


@router.delete("/api/assistants/{entry_id}", response_model=AssistantEntryList)
def delete_assistant_entry(project: CurrentProject, entry_id: str) -> AssistantEntryList:
    with translate_errors():
        return project.delete_assistant_entry(entry_id)


@router.post("/api/assistants/order", response_model=AssistantEntryList)
def reorder_assistant_entries(project: CurrentProject, request: ReorderAssistantsRequest) -> AssistantEntryList:
    with translate_errors():
        return project.reorder_assistant_entries(request)


@router.post("/api/assistants/unlist", response_model=AssistantEntryList)
def unlist_assistant_entry(project: CurrentProject, request: UnlistAssistantRequest) -> AssistantEntryList:
    with translate_errors():
        return project.unlist_assistant_entry(request)
...
def list_chat_sessions(project: CurrentProject) -> ChatSessionList:
    with translate_errors():
        return project.list_chat_sessions()


@router.post("/api/chats", response_model=ChatSession)
def create_chat_session(project: CurrentProject, request: CreateChatSessionRequest) -> ChatSession:
    with translate_errors():
        return project.create_chat_session(request)
...
def save_chat_session(project: CurrentProject, chat_id: str, request: SaveChatSessionRequest) -> ChatSession:
    with translate_errors():
        return project.save_chat_session(chat_id, request)


@router.delete("/api/chats/{chat_id}", response_model=ChatSessionList)
def delete_chat_session(project: CurrentProject, chat_id: str) -> ChatSessionList:
    with translate_errors():
        return project.delete_chat_session(chat_id)
...
async def put_node(project: CurrentProject, node_id: str, request: Request):
    ...
    kind = project.lookup_node_kind(node_id)
    if kind is None:
    ...
    if kind == "assistant":
        ...
        if err:
            raise HTTPException(status_code=400, detail=err)
    with translate_errors():
        return project.save_node(node_id, parsed)


@router.delete("/api/nodes/{node_id}", status_code=204)
def delete_node(project: CurrentProject, node_id: str):
...
def get_todos(project: CurrentProject) -> TodoDocument:
    with translate_errors():
        return project.read_todos()


@router.get("/api/todos/embedded", response_model=EmbeddedTodoList)
def get_embedded_todos(project: CurrentProject) -> EmbeddedTodoList:
    """The rebuildable index of in-prose embedded todos across all scenes."""
    with translate_errors():
        return project.read_embedded_todos()


@router.post("/api/todos", response_model=TodoDocument)
def create_todo(project: CurrentProject, request: CreateTodoRequest) -> TodoDocument:
    with translate_errors():
        return project.create_todo(request)


@router.patch("/api/todos/{todo_id}", response_model=TodoDocument)
def update_todo(project: CurrentProject, todo_id: str, request: UpdateTodoRequest) -> TodoDocument:
    with translate_errors():
        return project.update_todo(todo_id, request)


@router.delete("/api/todos/{todo_id}", response_model=TodoDocument)
def delete_todo(project: CurrentProject, todo_id: str) -> TodoDocument:
    with translate_errors():
        return project.delete_todo(todo_id)


@router.post("/api/references/resolve", response_model=ReferenceResolveResponse)
def resolve_references(project: CurrentProject, request: ReferenceResolveRequest) -> ReferenceResolveResponse:
    with translate_errors():
        return project.resolve_references(request.ids)
...
def list_reference_candidates(
    ...
    with translate_errors():
        return project.list_reference_candidates(kind=kind, entry_type=entry_type, exclude_id=exclude_id)


@router.get("/api/references/graph", response_model=ReferenceGraphResponse)
def reference_graph(project: CurrentProject) -> ReferenceGraphResponse:
    with translate_errors():
        return project.reference_graph()
