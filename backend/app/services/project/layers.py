...
class LayerWalkMixin:
    ...
    def _metadata_schema_layer_paths(
        ...
        nearer-wins (`_merge_metadata_schema_layer`). `None` is the whole
        chain, which is what every resolution-scope caller wants and gets
        unchanged.
    ...
    def _truncate_layers_at(
        ...
        An id absent from the chain is a 404 — an authoring layer that is not
        on this project's walk is an error rather than a silently full chain.
        """
