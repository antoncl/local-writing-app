...
class _HelperFixtureBase(unittest.TestCase):
    ...
    def _update_scene_with_mutations(
        self, scene_id: str, *, title: str, entry_type: str, metadata: dict, body: str
    ) -> dict[str, tuple[str, str]]:
        """Like `_update_scene`, but `body` is legacy-grammar markers moved
        into a set + anchor (ADR-0095) before the scene is saved with them."""
        self._update_scene(scene_id, title=title, entry_type=entry_type, metadata=metadata, body="")
