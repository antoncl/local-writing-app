...
class TextAppendMutationTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.service = open_test_project(self.root, "Text Append Tests")
    ...
    def test_text_add_appends_with_a_space(self) -> None:
        scene = self._new_scene("S2")
        self._convert(scene, self._marker("rank", "add", "(acting)", "a1"))
        self.assertEqual(
            self.service.effective_state(self.honor, scene), {"rank": "Sergeant (acting)"}
        )

    def test_long_text_add_appends_a_paragraph(self) -> None:
        scene = self._new_scene("S2")
        self._convert(scene, self._marker("notes", "add", "Now%20writes%20in%20cipher.", "a1"))
        self.assertEqual(
    ...
    def test_intrinsic_body_append_uses_stored_body_as_base(self) -> None:
        scene = self._new_scene("S2")
        self._convert(scene, self._marker("body", "add", "She%20trusts%20the%20detective.", "a1"))
        self.assertEqual(
            self.service.effective_state(self.honor, scene),
    ...
    def test_intrinsic_title_append_uses_stored_title_as_base(self) -> None:
        scene = self._new_scene("S2")
        self._convert(scene, self._marker("title", "add", "the%20Bold", "a1"))
        self.assertEqual(
            self.service.effective_state(self.honor, scene), {"title": "Honor the Bold"}
        )
    ...
    def test_live_replace_resets_base_then_adds_append(self) -> None:
        ...
        self.assertEqual(
            self.service.effective_state(self.honor, scene), {"rank": "Captain (brevet)"}
        )
    ...
    def test_earlier_scene_sees_no_append(self) -> None:
        ...
        scene = self._new_scene("S2")
        self._convert(scene, self._marker("rank", "add", "(acting)", "a1"))
        self.assertEqual(self.service.effective_state(self.honor, s1), {})
