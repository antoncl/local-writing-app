...
- **App-set state … while this reference is set** — on a Select only: one option
  ...
  the Entry's panel and is never the default. Setting it to none on a type
  that inherits the rule from a layer above switches the rule off from this
  layer down — the same goes for clearing an inherited default or description.
