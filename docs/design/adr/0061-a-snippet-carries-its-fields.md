...
- Depends on: Jinja's **`{% include %}`** directive — a `snippet` (the reusable-text kind) is pulled into another prompt's body by including it by name. This ADR adds only the *field-definition* semantics to that include; the directive itself is the template-language thread's (ADR-0060 area). `{% include %}` already renders the snippet's text inline, so carrying the snippet's *fields* alongside that text is a natural extension, not a bolt-on.
...
A prompt declares its `inputs` on its front-matter and references them in its Jinja body as `{{ input.<name> }}` (ADR-0054). A **snippet** is a prompt with no invocation contract — reusable text pulled into another prompt's body by name (`{% include "house_voice" %}`).
...
A prompt's effective input set is its own declared `inputs` plus, recursively, the `inputs` of every snippet it `{% include %}`-includes. A snippet that includes another snippet contributes that one's inputs too.

### 2. Fields are gathered statically, before render — always

The input form must exist **before** the template runs: the user fills it, *then* the body renders. So effective inputs come from a **static scan** of literal `{% include "name" %}` tags in the body, recursively — not from executing the body.
...
- **A dynamically-named snippet contributes nothing.** `{% include input.x %}` cannot be resolved statically, so it adds no fields. **Literal names are the contract** for field inheritance.
...
### 5. A `{% include %}` inclusion is a reference edge, so dependents are alertable

Because a chat persists its input **values** and, transitively, its snippets, editing a snippet's fields can invalidate a resumable chat or an including prompt. A `{% include %}` reference is recorded as a **graph edge** (ADR-0040's extraction), so the reverse index can answer *"what includes this snippet?"* — and the editor **warns on edit**: *"used by N prompts / M chats — changing these fields may affect them."* The warning is advisory (it never blocks the edit); its obligation is to name the count truthfully.
...
- **Not a new input type or a new template directive.** This is a *resolution rule* over the existing `inputs` model and the `{% include %}` directive (owned by the template-language thread). Field-gathering adds no syntax.
- **Field-gathering never depends on render-time state.** The form is a static function of the body's literal `{% include %}` tags — never of what a conditional evaluates to (§2), and never a runtime signal (unlike ADR-0057's lore gate).
...
- **`{% include %}` becomes an extracted reference edge** (ADR-0040), so *"what includes this snippet?"* is a reverse-index lookup.
...
- **S1 — the resolver.** Static `{% include %}` scan → transitive effective-inputs union, nearer-wins merge, cycle/depth guard, in one backend function. Unit-tested (no UI). Every existing surface switches to reading it; behaviour for prompts *without* snippet-inputs is unchanged.
- **S2 — the reference edge + conflict surfacing.** Record `{% include %}` as an edge (dependency lookups light up); surface the same-name/different-type conflict in the preview.
