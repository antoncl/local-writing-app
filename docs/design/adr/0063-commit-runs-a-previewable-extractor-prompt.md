...
- **S3's "field contract is authored Jinja" is coupled to ADR-0065's S3.** Once prompt types collapse to `{general, snippet}`, `output.commit.fields` has no per-behaviour type to live on, so the field scope *must* move into the extractor Jinja. You cannot finish either ADR's S3 without the other — they are one piece of work. The mechanism (an authored field-contract fragment rendered at chat-start **and** commit, via a new `field_contract` helper) is **ADR-0067**.
- **The extractor is a `general` + `headless` prompt, not a new type.** S2 (#1174) shipped a `prompt:extractor` entry_type — **withdrawn** (ADR-0065 Amendment 1 §3): it re-created the sub-type proliferation ADR-0065 collapses. #1174 is re-based onto ADR-0067 S3, not merged as-is; its built-in `default-extractor.md` becomes a `general` prompt whose body is one `field_contract` call.
- **The two-step field list is realised, not deferred.** This ADR's Preview-shows-the-contract, plus the recurring "state the fields at chat-start and re-assert at commit" intent, is delivered by ADR-0067 §2 (one authored fragment, two renders — identical by construction, so it can't drift).

Net: S1 (`commit.target`) stands as shipped; **S2/S3 are superseded by ADR-0067's slice plan.**
