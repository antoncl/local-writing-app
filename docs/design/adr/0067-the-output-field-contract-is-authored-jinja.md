...
**Decision.** The commit stays a continuation of the chat — same system prompt, so the registered field set is read back exactly as S2 intends, and the transcript is posted as ordinary history. But the commit turn's lore selection narrows to the chat's **explicit `use()` picks only**: the entry under revision (`use(e)` in `revise-entry`) and any author-picked "Relevant lore". The implicit world selection (`use_lore()` → scene refs, journal, one-hop fan-out, always-in-context policy) is not sent on the commit turn, and the commit turn runs no journal detection. Transcription needs the transcript and the current entry, not the world. Mechanically: a `lore_mode` on the chat-turn assembly (`"implicit"` for an ordinary turn, `"used"` for the commit; a closed `Literal`, not a free string), threaded from the extraction through `run_chat_turn` to `expand_and_prepare_chat_blocks`, which branches once: the ordinary turn takes today's path unchanged; the read-only turn renders the picks through the one selector's `"used"` mode in its untiered one-shot form and touches no chat state — no detection, no baseline promotion, no seen-revisions save. No second selector, no second placement path.

**Why not the fresh pass.** ADR-0051 S4's fresh pass would also drop the lore, but it re-renders a separate contract and loses the read-back guarantee S2 bought. The continuation with a narrowed selection keeps both: the contract is read back, and the context is small.

**Consequences.** The "cached system prefix" cost argument for the continuation is retired — the commit turn shares the system prompt with the chat but not the lore blocks, so it is a cache miss on the lore either way (in practice it was already a miss: OpenRouter/DeepSeek reported zero cached tokens on the failing call). The commit's own provider calls are now recorded server-side as their own `ai_invocations` rows with their own usage (#1872), instead of a client cost delta the save path pinned on the transcript's last message. That retires the delta *for the commit only*; the streamed turn still round-trips one, and finishing the move is #1877.

**Anti-goals.**
- Not a prompt-side fix. A stronger cue was tested and does not help; the failure is context size, not instruction clarity.
- Not a transcript-grounding validator on the proposal (e.g. "the title must appear in the conversation"). It may still be worth adding as a *warning* on the card; it is not the fix.
- Not a change to what an ordinary turn sends. The same unbounded selection sits in front of every ordinary turn of that chat and can degrade any brainstorm turn on a cheap model — silently, as a worse answer; the commit was merely where it was caught. Bounding it is deferred, not rejected: #1876.
