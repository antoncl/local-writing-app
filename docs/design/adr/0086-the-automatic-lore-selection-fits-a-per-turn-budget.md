...
- **Amendment 1 (2026-09-12, #1887) — a direct mention re-journals an entry the journal knows only through the hop.** As accepted, the anti-goal "not a change to the journal or to detection" left one gap that §2b opened: the journal dedups by id (`expand_context`'s `in_scope`), so an entry that first entered as `depth1_expansion` kept that source forever — and under `named`, an author who later typed its name still did not get it, with nothing in the report to say so. This amendment narrows the anti-goal by one rule: an id the journal holds *only* as `depth1_expansion` does not keep a **direct** mention (the author's message, the rendered prompt, the scene's prose) out; naming it appends a second entry for the same id under the direct source and that turn. The journal stays append-only and still records every detection; it may now hold one id twice, and every reader keys entries by `(id, source, turn)`. The selector already keeps the best-ranked candidate per id (§1), so the promoted entry is what `named` sends and what the fit ranks. A depth-1 re-hit of a journaled id still adds nothing, and a direct re-mention of a directly journaled id still adds nothing — "refresh on re-mention" for the tie-break stays rejected (Alternatives). Accepted with the shipping PR.
...
**Fit order** within the inferred set is the distance from the author's own words: what the author typed this session, then what the author's prompt named, then what the anchored scene's prose names, then what the app found one hop from those, then what the app followed through graph edges. Within one source, the entry journaled latest first (`added_at_turn` descending — the turn it was *first* noticed under that source; the journal records no re-mentions of a directly journaled entry, and by Amendment 1 exactly one promotion of a hop-journaled one), then by id. An id journaled twice is one candidate: the entry that fits first wins. Structural-hop ids carry no turn and order by id. The key is `(source, -added_at_turn, id)`: total, deterministic, no title involved.
...
- **Refresh `added_at_turn` on re-mention** so "latest first" means last mentioned. Not taken here: it changes the journal's semantics (ADR-0075) for a tie-break; the order is stated honestly as first-noticed and can be revisited with evidence. (Amendment 1 does not reopen this: it appends across the hop/direct line only, where the difference is inclusion under `named`, not a tie-break.)
