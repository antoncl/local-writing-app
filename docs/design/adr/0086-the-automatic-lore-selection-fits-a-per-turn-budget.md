...
- **Amendment 1 (2026-09-12, #1887) — the journal admits a better-ranked re-mention.** As accepted, the anti-goal "not a change to the journal or to detection" left a gap §1 and §2b opened: the journal dedups by id (`expand_context`'s `in_scope`), so an entry kept the *first* source it entered under forever. Under `named`, an entry that arrived through the hop was withheld even after the author typed its name; under the budget, an entry the scene's prose named on turn 0 was still fitted as `scene_prose` on the turn the author named it, and left out with that label. This amendment narrows the anti-goal by one rule, **monotone precedence**: a mention from a source ranked strictly better than any the journal already holds for that id (§1's order: the author's message, then the prompt, then the scene's prose, then the hop) appends one more entry for that id under that source and that turn; a mention from the same or a worse rank adds nothing. So a hop can never re-add, a same-source re-mention never refreshes ("refresh on re-mention" stays rejected — Alternatives), and the journal holds at most one entry per `(id, source)` — that pair, not the id, is an entry's identity for every reader. The journal stays append-only and still records every detection. The selector keeps the best-ranked candidate per id (§1), so the promoted entry is what `named` sends and what the fit ranks; the door's roster counts entities, not entries, and marks a promoted line *named*. ADR-0075 §6 is amended by reference. Accepted with the shipping PR.
...
- **Not a change to the journal or to detection.** ADR-0075's journal keeps recording every detection, append-only, with its `added_at_turn` as today (a re-mention does not refresh it — the journal has no last-seen, and adding one is not this decision). The budget shapes the *send*, never the record of what was noticed. *Narrowed by Amendment 1:* a mention from a better-ranked source than any the journal holds for an id appends one more entry under that source; nothing is refreshed or removed.
...
**Fit order** within the inferred set is the distance from the author's own words: what the author typed this session, then what the author's prompt named, then what the anchored scene's prose names, then what the app found one hop from those, then what the app followed through graph edges. Within one source, the entry journaled latest first (`added_at_turn` descending — the turn it was *first* noticed under that source; the journal holds at most one entry per `(id, source)`, a better-ranked re-mention being a second entry by Amendment 1), then by id. An id journaled under more than one source is one candidate: the entry that fits first wins. Structural-hop ids carry no turn and order by id. The key is `(source, -added_at_turn, id)`: total, deterministic, no title involved.
...
- **Refresh `added_at_turn` on re-mention** so "latest first" means last mentioned. Not taken here: it changes the journal's semantics (ADR-0075) for a tie-break; the order is stated honestly as first-noticed and can be revisited with evidence. (Amendment 1 does not reopen this: a re-mention from the *same* source still adds nothing; only a strictly better-ranked source appends, and that changes rank, not the tie-break.)
