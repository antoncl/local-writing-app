...
| `fields(x)` | The **full** field roster of a node or an `entry_type` FQN — a list of descriptors, each `{id, label, type, options, description, group, proposable}` (plus `items` for list fields; `tag_vocabulary` for a proposable tag-vocabulary field). `group` is the field's section label (an applied struct like `GMO`, or a manual header; `None` when ungrouped), so a template can group by it: `{% for f in fields(e) if f.group == "GMO" %}`. `proposable` is advisory; the template chooses what to show. `tag_vocabulary` is present only on a proposable tag-vocabulary `entity_ref_list` field (ADR-0082 §2) — the entry's existing tags, by usage, for a "prefer tags that already exist" prompt to point at. |
...
| `use(node, "stable"\|"volatile", snapshot=id)` | `""` — adds the node to what the AI sees. It prints nothing where you write it: the app fetches the node and includes it for you, with no duplicates. Hand it a whole picker selection to add every pick at once. The optional `"stable"`/`"volatile"` hint nudges caching and can usually be left off. It does not turn automatic lore on — pair it with `auto_lore()` for that. `snapshot=id` places the entry as it was at that snapshot (an ADR-0087 snapshot id), alongside the live entry if you also `use()` it; an empty snapshot is the plain call. The template never reads the snapshot — the app places it. |
