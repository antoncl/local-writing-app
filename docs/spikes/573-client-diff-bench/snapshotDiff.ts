// Faithful TypeScript port of the backend snapshot diff, for the #573 benchmark:
//   backend/app/services/project/snapshot_diff.py  (the pipeline + 3 SequenceMatcher uses)
//   backend/app/services/markdown_scan.py          (the construct scanner)
//   Python difflib.SequenceMatcher                 (ported from scratch; autojunk always False)
//
// Goal is byte-for-byte parity of the produced runs with the Python engine, so the
// timing comparison measures the SAME algorithm. Authored in TS and run natively on
// Node's type-stripping — the same V8 the browser runs — so numbers are representative.
//
// Caveat: offsets are UTF-16 code units (Python uses code points). Identical for BMP
// text, which is all fiction prose and the whole corpus. Astral chars (emoji) would
// need code-point-aware offsets; out of scope for a perf feel.

export type Kind = "equal" | "now" | "was";
export interface DiffRun { kind: Kind; text: string; stacked: boolean; }
type Interval = [number, number];
type Region = [number, number, number, number]; // was_start, was_end, now_start, now_end

const run = (kind: Kind, text: string, stacked = false): DiffRun => ({ kind, text, stacked });
...
class SequenceMatcher {
  ...
  private isbjunk(elt: string): boolean { return this.bjunk.has(elt); }
...
function scanReferenceLink(block: string, i: number): Scan {
  ...
  if (!m) { REFERENCE_DEF_Y.lastIndex = i; m = REFERENCE_DEF_Y.exec(block); }
  if (!m) return null;
...
function protectedIntervals(block: string): Interval[] | null {
  ...
  while (index < length) {
    ...
    if (scanned === null) { index++; continue; }
...
function linkEnd(block: string, start: number): number | null {
  ...
  while (index < block.length && depth) {
    if (block[index] === "\\") { index += 2; continue; }
...
function pairDelimiters(delimiters: [number, number, string][]): Interval[] | null {
  ...
  for (const [start, end, marker] of delimiters) {
    ...
    if (!stack) { stack = []; openRuns.set(marker, stack); }
    if (stack.length) {
...
function isStructured(block: string): boolean {
  const lines = block.split("\n");
  return lines.some((l) => LINE_MARKER.test(l) || TABLE_DELIMITER.test(l) || SETEXT_UNDERLINE.test(l)) || isCodeBlock(block);
}
...
function reassembles(runs: DiffRun[], was: string, now: string): boolean {
  return (
    runs.filter((r) => r.kind !== "now").map((r) => r.text).join("") === was &&
    runs.filter((r) => r.kind !== "was").map((r) => r.text).join("") === now
  );
}
...
function expandOutOfConstructs(regions: Region[], wasIntervals: Interval[], nowIntervals: Interval[]): Region[] {
  ...
  while (changed) {
    ...
    for (const region of regions) {
      ...
      if (wider[0] !== region[0] || wider[1] !== region[1] || wider[2] !== region[2] || wider[3] !== region[3]) changed = true;
...
function settle(
  regions: Region[], was: string, now: string, wasIntervals: Interval[], nowIntervals: Interval[]
): Region[] | null {
  for (let pass = 0; pass < SETTLE_PASSES; pass++) {
    regions = expandOutOfConstructs(regions, wasIntervals, nowIntervals);
    const aligned: Region[] = [];
    let wasCursor = 0, nowCursor = 0, changed = false;
    for (let [wasStart, wasEnd, nowStart, nowEnd] of regions) {
      ...
      if (wasCursor + common !== wasStart || nowCursor + common !== nowStart) {
        wasStart = wasCursor + common; nowStart = nowCursor + common;
        changed = true;
      }
      aligned.push([wasStart, wasEnd, nowStart, nowEnd]);
      wasCursor = wasEnd; nowCursor = nowEnd;
    }
