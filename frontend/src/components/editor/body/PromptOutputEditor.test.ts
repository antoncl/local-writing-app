...
import type { MetadataSchema, PromptContextStrategy } from "@/lib/types";
...
const SCHEMA = {
  ...
  entry_types: {
    ...
    "lore:character": {
      ...
      kind: "lore",
      parent: "lore:base",
      fields: ["summary", "secret_field", "portrait"],
    },
  },
  fields: {
    summary: { name: "Summary", type: "long_text", options: [] },
    secret_field: { name: "Secret", type: "text", options: [], ai_proposable: false },
    portrait: { name: "Portrait", type: "entity_ref", options: [] },
  },
...
describe("PromptOutputEditor (ADR-0062 D3)", () => {
  ...
  it("read-only locks every control and swallows edits", async () => {
    ...
    const onChange = vi.fn();
    render(PromptOutputEditor, { props: { contextStrategy: preset, metadataSchema: null, readOnly: true, onChange } });
