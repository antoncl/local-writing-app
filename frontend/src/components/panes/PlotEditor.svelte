...
<script lang="ts">
  ...
  // reconcile that froze the menu open for ~10s (#1095 → #1100). So no board state or
  // per-node signal here.
...
<style>
  ...
  /* Lift a node above its siblings while its kebab menu is open (#1095/#1100). The menu
     ...
     its `zIndex` field; an `!important` rule beats that inline value — but only while the
     menu element exists, reverting the instant `{#if menuOpen}` unmounts it. Pure CSS, so
     nothing touches the bound `flowNodes` array (reassigning it to bump one node forced a
