<script lang="ts">
  ...
  // handler that needs it is wired. Also exposes a trigger for the add-menu
  // popover tests (#1839). Kept out of the app; imported by
  ...
  let list: ViewNodeList<EvalNode> | undefined = $state();
</script>

<button type="button" class="fixture-add-trigger" onclick={(e) => list?.toggleAddMenu(null, "root", e)}>+</button>
<ViewNodeList bind:this={list} {result} row={rowSnippet} addMenu={withAddMenu ? addMenuSnippet : undefined} />
