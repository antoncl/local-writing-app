...
// per-pane draft/autosave lifecycle, the embedded-TODO bridge, and tearing
// panes down. Extracted from App.svelte (#14 P0), the last and largest of the
// god-shell slices. The pure draft *semantics* live in
...
// + domain stores. Resolving/claiming a pane and seeding its drafts — every
// `openX` opener plus the pane-acquisition helpers — lives next door in
// editorPaneOpen.ts (the file-size guard); this file keeps thin delegates.
//
...
import {
  ...
  openProjectNode as runOpenProjectNode,
  openScene as runOpenScene,
  openStructureNode as runOpenStructureNode,
  openChat as runOpenChat,
  openPrompt as runOpenPrompt,
  ...
  duplicateView as runDuplicateView,
  openLore as runOpenLore,
...
// Re-exported for existing importers (editorPanes.loadFailure.test.ts,
// editorPanes.navigation.test.ts) — the string itself, and the opener that
// throws it, now live in editorPaneOpen.ts.
export { FOREIGN_PROJECT_NODE };
...
class EditorPanesController {
  ...
  // Pane acquisition, every per-kind opener, and `openNodeOfKind`'s cross-kind
  // dispatch — "resolve or claim a pane and seed its drafts" — extracted to
  // editorPaneOpen.ts (the file-size guard); these stay as thin delegates so
  // the public surface (and every existing call site / test) is unchanged.
  openProjectNode(expectedId?: string): Promise<void> {
    return runOpenProjectNode(this, expectedId);
  }

  openScene(sceneId: string): Promise<void> {
    return runOpenScene(this, sceneId);
  }

  openStructureNode(nodeId: string): Promise<void> {
    return runOpenStructureNode(this, nodeId);
  }

  openChat(chatId: string): Promise<void> {
    return runOpenChat(this, chatId);
  }

  openPrompt(entryId: string): Promise<void> {
    return runOpenPrompt(this, entryId);
  }

  openPlotTemplate(entryId: string): Promise<void> {
    return runOpenPlotTemplate(this, entryId);
  }

  openPlotCard(entryId: string): Promise<void> {
    return runOpenPlotCard(this, entryId);
  }
  ...
  openAssistant(entryId: string): Promise<void> {
    return runOpenAssistant(this, entryId);
  }

  openView(viewId: string): Promise<void> {
    return runOpenView(this, viewId);
  }

  createAndOpenView(kind: string): Promise<void> {
    return runCreateAndOpenView(this, kind);
  }

  duplicateView(spec: ViewSpec, title: string): Promise<void> {
    return runDuplicateView(this, spec, title);
  }

  openLore(entryId: string): Promise<void> {
  ...
  openResearchNote(noteId: string): Promise<void> {
    return runOpenResearchNote(this, noteId);
  }
