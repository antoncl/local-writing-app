...
import type { Scene, Snapshot } from "@/lib/types";
...
export class SnapshotStripController {
  ...
  async park(snapshotId: string | null): Promise<void> {
    ...
    try {
      const detail = await api.readSnapshot(sceneId, snapshotId);
      const html = await sceneMarkdownToHtml(detail.body);
      if (fresh()) this.bodyHtml = html;
    } catch {
